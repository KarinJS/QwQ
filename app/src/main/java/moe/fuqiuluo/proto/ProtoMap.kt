...
import moe.qwq.miko.ext.jsonObject
...
class ProtoMap(
    ...
    override fun get(vararg tags: Int): ProtoValue {
        ...
        tags.forEachIndexed { index, tag ->
            if (index == tags.size - 1) {
                return curMap[tag] ?: error("pb[${tags.joinToString(", ")}] Index: $index, Tag: $tag not found")
            }
            curMap[tag]?.let { v ->
                if (v is ProtoMap) {
                    curMap = v.value
            ...
            } ?: error("pb[${tags.joinToString(", ")}][$index] Index: $index, Tag $tag not found")
        }
